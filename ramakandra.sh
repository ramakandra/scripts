...
while read ip; do
    ...
    if [[ $proto == "tcp" || $proto == "all" ]]; then 
        ...
        if [[ ! -z $ports ]]; then 
            ...
            nmap -e ${iface} ${nmap_opt} -oX ${log_dir}/ndir/${ip}-tcp.xml -oG ${log_dir}/ndir/${ip}-tcp.grep -p ${ports} ${ip} --script=http-headers.nse
        else
...
echo -e "${GREEN}[+]${RESET} Running Whatweb"
echo ""
echo ""
...
then
	...
	echo -e "${RED}[!]${RESET} Skipped: whatweb: Port 80 not found in scan"
fi
...
then
	...
	nikto -h ${ip}
        ...
        echo -e "${RED}[!]${RESET} Skipped: nikto: Port 80 not found in scan"
fi
